 ...
 *      Collections.synchronizedList 返回一个同步代理类：synchronized同步块！
 *      自己实现ArrayList的子类，并进行同步、加锁
 *
 * CopyOnWriteArrayList（jdk1.5）用于处理“读多写少”的并发场景
 *
 * 1.用ReentrantLock同步写操作
 *
 * 2.CopyOnWriteArrayList对元素进行迭代时，仅仅返回一个当前内部数组的快照，也就是说，
 * 如果此时有其它线程正在修改元素，并不会在迭代中反映出来，因为修改都是在新数组中进行的。
 * 迭代器对象也不支持修改方法，全部会抛出UnsupportedOperationException异常
 *
 * 3.数组内存占用的太大，那么可能会造成频繁GC,所以CopyOnWriteArrayList并不适合大数据量的场景
 *
 * 4.数据一致性
