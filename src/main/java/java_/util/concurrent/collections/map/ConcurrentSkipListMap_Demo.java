...
/**
 * 并发跳表：
 * ConcurrentSkipListMap的由来：JDK1.6时，为了对高并发环境下的有序Map提供更好的支持。相当于并发版本的TreeMap。
 *
 ...
 *
 * 跳表，跳跃表：类似于链表的结构，但是其查询、插入、删除的时间复杂度都是 O(logn)
 * 跳表由很多层组成；
 * 每一层都是一个有序链表；
 * 对于每一层的任意结点，不仅有指向下一个结点的指针，也有指向其下一层的指针。
 *
 * 注意：ConcurrentSkipListMap在删除键值对时，不会立即执行删除，而是通过引入“标记结点”，以“懒删除”的方式进行，以提高并发效率。
